...
RUN pip install wrapt --upgrade --ignore-installed \
    gym gym-minigrid pyopengl pylint natsort kfp seaborn polars \
    transformers==4.24.0 librosa \
    numba==0.48 --ignore-installed \
    git+https://github.com/h2oai/datatable \
    fugashi==1.1.0 ipadic==1.0.0
...
RUN pip install \
    ipywidgets widgetsnbextension jupyterlab-lsp 'python-lsp-server[all]'
